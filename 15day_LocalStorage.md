...
// input form과 list를 선택함.
      ...
      function addItem(e) {
        // 새로고침 방지
        e.preventDefault();
        // name이 item인 태그의 값을 text로 선언한다.
        const text = this.querySelector("[name=item]").value;
        // item에 text와 체크되었는지 확인할 수 있는 변수 done을 넣는다.
        const item = {
          text,
          done: false
        };
        // 배열에 item을 추가한다.
        items.push(item);
        // populateList함수의 인자로 items와 itemsList를 넣는다.
        populateList(items, itemsList);
        //로컬저장소에 items 라는 키와 JSON.stringify(items)라는 값으로 저장한다.
      ...
      function toggleDone(e) {
        ...
        // done이 false면 true로, true이면 false로 바꾼다.
      ...
      // submit 할때 addItem 함수를 실행해서 아이템을 추가한다.
      addItems.addEventListener("submit", addItem);
      // 목록을 클릭하면 toggleDone 함수가 실행되면서 앞에 아이콘이 추가된다.
      itemsList.addEventListener("click", toggleDone);

      populateList(items, itemsList);
```

- 데이터베이스를 사용하지 않고 가볍게 사용해보기에 좋은 것 같다.
